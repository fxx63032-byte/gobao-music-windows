  ...
  for(let i=0;i<4;i++) {
    const result=await run(`(async()=>{
      ...
      while(performance.now()-start<12000) {
        ...
        if(video.readyState>=2 && video.currentTime>0.1 && !video.paused&&backdrop&&backdrop.dataset.frameReady==='true'&&Number(backdrop.dataset.frameCount)>0)break;
        await new Promise(r=>setTimeout(r,80));
      }
      if(video.readyState<2 || video.paused || video.currentTime<=0.1 || !backdrop || backdrop.dataset.frameReady!=='true' || Number(backdrop.dataset.frameCount)<1)throw new Error('Landscape composite did not play');
      await new Promise(r=>setTimeout(r,400));
      const initialFrameCount=Number(backdrop.dataset.frameCount);
      video.currentTime=Math.max(0,video.duration-0.15);
      await new Promise(r=>setTimeout(r,800));
      const finalFrameCount=Number(backdrop.dataset.frameCount);
