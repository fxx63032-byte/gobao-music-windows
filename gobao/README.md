...
- 四个视频卡片直接显示在 `DIY → 常用 → GO宝动态背景` 顶部；完整素材库入口仍保留在背景媒体设置中。点击即可切换、静音循环、完整居中显示，并沿用上游设置持久化。
...
Windows Electron smoke 通过实际本地 HTTP 服务加载主程序 HTML，检查四张卡片位于“常用”页第一组、四段视频解码、时间推进、循环、静音、快速切换、完整显示、设置恢复、清除背景及光晕响应。登录检查验证大小眼门禁不再出现、自动弹窗被关闭、QQ/网易云入口可见以及汽水登录入口隐藏。光晕检查使用确定性输入，不代表真实歌曲端到端节拍精度验收。运行结果和截图写入 `verification/`，由 Actions 上传。
